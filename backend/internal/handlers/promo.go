...
func GetPromoDiscount(c *gin.Context) {
	rows, err := database.DB.Query(
		`SELECT id, code, discount_amount, is_active,
		        COALESCE(usage_limit,0), COALESCE(use_count,0), updated_at
	...
	for rows.Next() {
		...
		rows.Scan(&p.ID, &p.Code, &p.DiscountAmount, &p.IsActive,
...
// UpdatePromoDiscount lets admin change discount amount, usage limit and active flag.
...
func UpdatePromoDiscount(c *gin.Context) {
	...
	q := `UPDATE promo_discount
	      SET discount_amount=$1, is_active=$2, usage_limit=$3, updated_at=CURRENT_TIMESTAMP`
	args := []interface{}{req.DiscountAmount, active, limit}
	if req.ResetCount {
		q += `, use_count=0`
	}
	q += ` WHERE id=$4`
	...
	).Scan(&p.ID, &p.Code, &p.DiscountAmount, &p.IsActive,
		&p.UsageLimit, &p.UseCount, &p.UpdatedAt)
